...
def Silhouette(_centroid, _assignment, _data):
    ...
    for c in range(num_of_clusters):
        ...
        for i in range(len(_data["x"]) - 1):
            if data["cluster"][i] == c:
                a_i_list = [distance((data["x"][i], data["y"][i]),(_data["x"][k], _data["y"][k])) for k in range(len(_data["x"]) - 1) if ((data["cluster"][k] == c) and (k != i))]
                b_i_list = [distance((data["x"][i], data["y"][i]),(_data["x"][k], _data["y"][k])) for k in range(len(_data["x"]) - 1) if ((data["cluster"][k] != c) and (k != i))]
        a_i = sum(a_i_list) / (count - 1)
        b_i = sum(b_i_list) / (len(_assignment) - count)
        s_i = (b_i - a_i)/(max(a_i, b_i))
        s_i_list.append(s_i)
    final_s_i = sum(s_i_list) / num_of_clusters
